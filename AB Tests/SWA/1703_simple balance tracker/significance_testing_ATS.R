...
df1<-subset(df1, df1$Offer_Name=="Simple Balance Tracker" |  df1$Offer_Name=="Control")
...
ad.test(subset(df1, df1$Offer_Name=="Simple Balance Tracker")$ATS)
cvm.test(subset(df1, df1$Offer_Name=="Simple Balance Tracker")$ATS)
shapiro.test(subset(df1, df1$Offer_Name=="Simple Balance Tracker")$ATS)

## Step 2: assess variance between Offer_Names

##look at variance
##df1
var(subset(df1, df1$Offer_Name=="Simple Balance Tracker")$ATS)
...
wilcox.test(df1$ATS~df1$Offer_Name, conf.level=0.05) #0.01606 not significant
t.test(log(df1$ATS)~df1$Offer_Name, conf.level=0.05, var.equal=FALSE) #0.02889 not significant
kruskal.test(ATS~Offer_Name, data=df1) #0.01606 not significant
